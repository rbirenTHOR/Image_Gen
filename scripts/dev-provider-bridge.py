...
class Handler(BaseHTTPRequestHandler):
 ...
 def handle_proxy(self):
  ...
  if u.scheme!='https' or not (u.hostname in ['api.openai.com','queue.fal.run','fal.media','rest.fal.ai'] or (u.hostname or '').endswith('.fal.media') or (u.hostname or '').endswith('.falusercontent.com')):self.send_error(403);return
  body=self.rfile.read(int(self.headers.get('Content-Length','0'))) if self.command in ['POST','PUT'] else None
  headers={k:self.headers[k] for k in ['Authorization','Content-Type','X-Fal-Object-Lifecycle'] if self.headers.get(k)}
