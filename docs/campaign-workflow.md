...
- Deliverables default to six shots for a new campaign. Selecting a preview, six-shot or 18-shot package sets the plan to those roles, instead of accumulating an unexpected number. Users can change shapes, reorder, remove or add roles. Free-form shot direction on an existing setup requires explicit customization. Use **Select all**, uncheck unwanted roles and **Generate N selected photos** once. All checked roles (up to the 30-shot plan limit) are persisted in one batch. Two images at a time are submitted automatically while the rest wait. Existing ready shots selected again create new versions. Package selection itself never generates or charges.
...
The generation endpoint reads the saved plan, validates references against ownership, validates all selected shot IDs and caps provider concurrency at two per campaign batch and records per-shot prompts/formats plus the full campaign snapshot before provider submission. Retries use those saved inputs. The client retains an unconfirmed generation request in session storage across reloads; Recover last request reuses its identifier and does not create a second paid batch.
...
The `waiting` job state persists not-yet-submitted selections in the same batch as active images, with the same immutable prompts, references and formats. Polling advances waiting jobs through an atomic database claim, enforcing both two per batch and eight active images per owner. Concurrent tabs cannot claim the same waiting image twice. Reopening the campaign resumes polling and remaining submissions; no browser-local loop or repeated generation clicks are needed. Closing the app pauses advancement of unsubmitted shots until the campaign is reopened. A failed provider request is shown individually and is never automatically resubmitted; other selected shots continue.
